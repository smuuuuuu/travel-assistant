...
public class AiAssistantController {
            ...
            return chatClient
                ...
                .doOnComplete(() -> {
                    if (sessionId != null && !sb.isEmpty()) {
                        ...
                        publisher.publishEvent(new AiMessageEvent(this, sessionId, questionWithImage, sb.toString(), msgUid,1));
                    }
                });
        } catch (Exception e) {
            throw new BusException(CodeEnum.SYSTEM_ERROR);
    ...
    public Flux<String> chat(@RequestParam(value = "message") String message,
        ...
        return chatClient.prompt(finalMessage)
                ...
                .doOnComplete(() -> {
                    // 2. 流结束时，发布事件
                    publisher.publishEvent(new AiMessageEvent(this,sessionId,message,sb.toString(),msgUid,0));
                })
                .doOnError(e -> {
                    // 处理异常情况
                    throw new BusException(CodeEnum.SYSTEM_ERROR);
